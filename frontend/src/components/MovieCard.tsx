...
const MovieCard :React.FC<MovieCardProps>= ({movie}) => {
  ...
  return (
    ...
    <div className="relative w-full md:w-1/3 h-56 md:h-auto">
      <Image
        src={movie?.poster_path ? `https://image.tmdb.org/t/p/w342${movie.poster_path}` : "/fallback.png"}
        alt={movie?.title}
        fill
    ...
    <CardContent className="flex flex-col p-4 md:w-2/3 gap-2">
      ...
      <div className='flex justify-end items-center gap-4'>
        ...
        <Button variant={isFavorite ? 'default' : 'secondary'} onClick={() => toggleFavorite(movie)}>
            <Star className={`${isFavorite ? 'fill-yellow-500 text-yellow-500' : 'text-gray-500'} size-6`} />
            <p>
